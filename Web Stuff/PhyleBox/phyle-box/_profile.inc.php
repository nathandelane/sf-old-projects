...
class _Profile_Page extends PhyleBoxPage {
	...
	public function _Profile_Page() {
		...
		if (!isset(self::$__explicity)) {
			$query = "select e.id, e.content from `pbox`.`explicity` e";
...
?>
<form id="profileForm" action="<?php echo $_SERVER["REQUEST_URI"]; ?>" method="post" enctype="multipart/form-data">
...
?>
	...
	<div class="column">
		<div class="contentBox" id="explicity">
			...
			<div class="formRow">
				<label for="siteExcplicity">Explicity*</label>
				<select name="siteExcplicity" id="siteExcplicity">
<?php
...
?>
					<option value="<?php echo $nextExplicity["id"]; ?>"<?php if (Strings::equals($nextExplicity["id"], $explicity)) { echo "selected=\"selected\""; } ?>>
						<?php echo $nextExplicity["content"]; ?>
					</option>
<?php
