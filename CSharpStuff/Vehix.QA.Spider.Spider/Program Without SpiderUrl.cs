...
namespace Vehix.QA.Spider.Spider
{
	class Program
	{
			...
			private void Background_HandleAsyncResult(IAsyncResult ar)
			{
				...
				try
				{
					...
					foreach (string u in fetch)
					{
						lock (__visitedPages)
						{
							if (__visitedPages.TryGetValue(u, out t))
							{
								t.Hit();
							}
							else
							{
								__visitedPages.Add(u, t = new UrlTracking(u));
					...
					WriteLogMessage(String.Format("{2} Request for {0} passed: {1} (title: {3}, actual url: {4})", this.Url, response.StatusCode.ToString(), this.Id, ((String.IsNullOrEmpty(pageTitle)) ? "null" : pageTitle), request.RequestUri.AbsoluteUri), true);
					Interlocked.Increment(ref __goodUrls);
				}
				catch (Exception ex)
				{
					this.Ex = ex;
					WriteLogMessage(String.Format("{2} Request for {0} failed: {1}", this.Url, ex.Message, this.Id), true);
		...
		/// </summary>
		private static Dictionary<string, UrlTracking> __visitedPages;
		...
		public Program()
		{
			...
			__visitedPages = new Dictionary<string, UrlTracking>();
			__goodUrls = 0;
			__badUrls = 0;
			...
			if (__regex.IsMatch(startingUrl))
			{
				...
				UrlTracking t = new UrlTracking(startingUrl);
				lock(__visitedPages) __visitedPages.Add(startingUrl, t);
