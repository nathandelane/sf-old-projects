...
namespace ExampleAxiomApplication
{
	public class ExampleApplication
	{
		...
		private class ConfigureConsole
		{
			...
			// Menu items
			private List<ConfigOption> _currentMenuItems;

			// Currently selected item
			private ConfigOption _currentOption;

			// Options for the currently selected system
			private List<ConfigOption> _currentSystemOptions;
			#endregion
			...
			{
				_currentMenuItems = new List<ConfigOption>();
				_currentSystemOptions = new List<ConfigOption>();
				...
				foreach ( RenderSystem r in Root.Instance.RenderSystems )
					_renderSystems.PossibleValues.Add( r.ToString() );
			...
			public DialogResult Show()
			{
				while ( true )
				{
					...
					// Build menu
					if ( _currentOption == null ) // Main-menu
					{

						// Build menu from m_currentOptions
						foreach ( ConfigOption c in _currentSystemOptions )
							_currentMenuItems.Add( c );
					}
					else // Option menu 
					{
						// Add possible values for this option
						foreach ( object value in _currentOption.PossibleValues )
							_currentMenuItems.Add( new ConfigOption( value.ToString(), "", false ) );
					}
			...
			private void BuildCurrentSystemOptions()
			{
				...
				_currentSystemOptions.Clear();

				// Add rendersystem options to the list
				_currentSystemOptions.Add( _renderSystems );

				// Browse and add options of current rendersystem
				foreach ( ConfigOption c in _currentSystem.ConfigOptions )
					_currentSystemOptions.Add( c );
			}
			...
			private DialogResult HandleNextKeyPress()
			{
				...
				if ( _currentOption == null )
				{
					...
					else if ( key == ConsoleKey.Enter )
					{
						...
						for ( int i = 0; i < _currentSystemOptions.Count; i++ )
						{
							ConfigOption opt = _currentSystemOptions[ i ];
					...
					else
					{
						// Check for selection
						int selection = 0;
						if ( IsDigit( ref selection, key ) && selection < _currentMenuItems.Count )
							_currentOption = (ConfigOption)_currentMenuItems[ selection ];
					}
				}
				else
				{
					...
					int selection = 0;
					if ( IsDigit( ref selection, key ) && selection < _currentOption.PossibleValues.Count )
					{
						// Check if we're about to change render system
						if ( _currentOption.Name == "Render System" )
						{
							_currentSystem = Root.Instance.RenderSystems[ selection ];
							_renderSystems = _currentOption;
							BuildCurrentSystemOptions();

							_currentOption = null; // Reset current option (to show main-menu)
						}
						else
						{
							_currentOption.Value = (string)_currentOption.PossibleValues[ selection ];

							// Set the selected value
							for ( int i = 0; i < _currentSystemOptions.Count; i++ )
								if ( _currentSystemOptions[ i ].Name == _currentOption.Name )
									_currentSystemOptions[ i ] = _currentOption;
			...
			private void DisplayOptions()
			{
				...
				if ( _currentOption != null && _currentOption.Name != null )
				{
				...
				int i = 0;
				foreach ( object o in _currentMenuItems )
				{
