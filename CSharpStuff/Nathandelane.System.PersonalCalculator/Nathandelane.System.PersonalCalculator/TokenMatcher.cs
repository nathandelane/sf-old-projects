...
namespace Nathandelane.System.PersonalCalculator
{
	static class TokenMatcher
	{
		...
		{
			{ TokenType.Addition, new Regex(String.Format("{0}(\\+){{1}}{0}", __numberFormat), RegexOptions.Compiled) },
			{ TokenType.And, new Regex(String.Format("{0}(&){{1}}{0}", __numberFormat), RegexOptions.Compiled) },
			{ TokenType.Assignment, new Regex(String.Format("{0}[=]{{1}}{1}", __variableFormat, __numberFormat), RegexOptions.Compiled) },
			{ TokenType.BinaryNumber, new Regex(String.Format("{0}", __binaryNumberFormat), RegexOptions.Compiled) },
			{ TokenType.ConversionFunction, new Regex(String.Format("([bohd]|rad|deg){{1}}{0}", __subExpressionFormat), RegexOptions.Compiled) },
			{ TokenType.DecimalNumber, new Regex(String.Format("{0}", __decimalNumberFormat), RegexOptions.Compiled) },
			{ TokenType.Division, new Regex(String.Format("{0}(/){{1}}{0}", __numberFormat), RegexOptions.Compiled) },
			{ TokenType.Function, new Regex(String.Format("{0}", __functionFormat), RegexOptions.Compiled) },
			{ TokenType.HexNumber, new Regex(String.Format("{0}", __hexNumberFormat), RegexOptions.Compiled) },
			{ TokenType.LastResult, new Regex("[\\$]{1}") },
			{ TokenType.Multiplication, new Regex(String.Format("{0}(\\*){{1}}{0}", __numberFormat), RegexOptions.Compiled) },
			{ TokenType.Modulus, new Regex(String.Format("{0}(%){{1}}{0}", __numberFormat), RegexOptions.Compiled) },
			{ TokenType.Negation, new Regex(String.Format("(-){{1}}{0}", __numberFormat), RegexOptions.Compiled) },
			{ TokenType.Number, new Regex(__numberFormat, RegexOptions.Compiled) },
			{ TokenType.NumericResult, new Regex(String.Format("^[-]{{0,1}}{0}$", __numberFormat), RegexOptions.Compiled) },
			{ TokenType.OctalNumber, new Regex(String.Format("{0}", __octalNumberFormat), RegexOptions.Compiled) },
			{ TokenType.Or, new Regex(String.Format("{0}(\\|){{1}}{0}", __numberFormat), RegexOptions.Compiled) },
			{ TokenType.Power, new Regex(String.Format("{0}(\\*\\*){{1}}{0}", __numberFormat), RegexOptions.Compiled) },
			{ TokenType.SpecialNumber, new Regex(__specialNumberFormat, RegexOptions.Compiled) },
			{ TokenType.SubExpression, new Regex(__subExpressionFormat, RegexOptions.Compiled) },
			{ TokenType.Subtraction, new Regex(String.Format("{0}(-){{1}}{0}", __numberFormat), RegexOptions.Compiled) },
			{ TokenType.TrigFunction, new Regex(String.Format("((sin|cos|tan){{1}}){0}", __subExpressionFormat), RegexOptions.Compiled) },
			{ TokenType.Variable, new Regex(String.Format("{0}", __variableFormat), RegexOptions.Compiled) },
			{ TokenType.Xor, new Regex(String.Format("{0}(\\^){{1}}{0}", __numberFormat), RegexOptions.Compiled) },
		};
